...
const NotificationBell = ({ token }: Props) => {
  ...
  const computeUnread = useCallback((data: AuditLog[]) => {
    const lastSeen = getLastSeen();
    if (!lastSeen) {
      // First load — mark everything as read silently
      if (data.length > 0) setLastSeen(data[0].created_at);
      return 0;
    }
    return data.filter((l) => new Date(l.created_at) > new Date(lastSeen)).length;
  }, []);

  const fetchLogs = useCallback(async () => {
    try {
      const data = await auditService.GETALL(token, undefined, MAX_SHOWN);
      if (data) {
        setLogs(data);
                ...
                return (
                  <div
                    ...
                    className={`px-4 py-3 flex items-start gap-3 transition-colors ${
                      i < Math.max(0, unread) ? "bg-green-50/60" : ""
                    }`}
                  >
                    <span
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-400 mt-0.5">
                        {log.user_id ? `Loja: ${log.user_id.slice(0, 8)}…` : "Sistema"}
                      </p>
                    </div>
